...
VehicleCommand QuadControl::GenerateMotorCommands(float collThrustCmd, V3F momentCmd) {
    ...
    // 1. get l. 45 degrees is a convention for some reason. (Does not make any sense)
    float l = L / (2.f * sqrtf(2.f));

    // 1. I will be using equation from exercise 1 3D Drone project

    // They labeled roters differently for some reason
    // w1 = w0, w2 = w1, w3 = w3, w4 = w2

    float F0 = collThrustCmd / 4.f + momentCmd.x / (4.f * l) + momentCmd.y / (4.f * l) + momentCmd.z + (4.f * kappa);
    float F2 = collThrustCmd / 2.f + momentCmd.x / (2.f * l) - F0;
    float F3 = F0 - momentCmd.x / (2.f * l) - momentCmd.y / (2.f * l);
    float F1 = collThrustCmd - F2 - F3 - F0;
